...
A _collection_ is a set, list, or map containing atomic values or structures.
...
A _structure_ is a set of labelled elements, each of which is an atomic value, a nested structure, or a collection.
...
A structure type might be assigned a static name.
footnote:[That is, the name is assigned to the type before the program using Jakarta Query is compiled and executed.]
...
- An _entity_ is an assignment of a name to a given type of record.
...
A _database_ is a set of records.
