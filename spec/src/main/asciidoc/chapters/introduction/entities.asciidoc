...
This specification does not require that implementations allow associations between entities, and the common language does not feature constructs for querying associations.
...
On the other hand, the persistence language defines constructs like <<joins-nested,joins>> and <<treat,typecasts>> which are intended for use in implementations which do support associations.
