...
It's not possible to uniquely assign compound labels to every element belonging to an arbitrary structure, since:

- a generic structure contains collections, and
- as discussed above in <<circularity>>, a database is not required to contain only trees.

However, we may use such labelling within any subgraph that does not contain a collection, though in principle the compound labels might not be unique.