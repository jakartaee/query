...
Evaluating a sequence of joins produces a result list in which each structure contains a labelled element corresponding to each join.
...
In this case, however, the restriction is only evaluated for elements of the result list which contain structures labelled by the alias `y`.
Any elements of the result list of the join which does not contain a structure labelled by the alias `y` is taken to satisfy the restriction, even when the predicate would not be satisfied by that element.
