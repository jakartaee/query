...
| `#sign#` | 1 | Numeric | Integer | Evaluates to the sign of the numeric value of its argument.
| `#mod#` | 2 | Both integer | Integer | Evaluates to the remainder when its first integer argument is divided by its second integer argument.
...
| `#size#` | 1 | Collection | Integer | Evaluates to the number of elements of the collection produced by evaluating its argument.
|===
...
**TODO** `EXTRACT()`, `INDEX()`, `VALUE()`, `KEY()`, `CAST()`, `TREAT()`, `FUNCTION()`
