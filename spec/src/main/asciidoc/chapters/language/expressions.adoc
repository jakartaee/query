...
NOTE: Typically, the last element of a path expression is assigned an <<atomic-values,atomic value type>>. Non-terminal path elements are assigned a <<structures-and-records,structure type>>, usually an embeddable type, if the element references an embeddable, or an entity type, if the element references an association. However, since a Jakarta Query provider is not required to support embedded attributes or associations, the implementation is not required to support compound path expressions.
