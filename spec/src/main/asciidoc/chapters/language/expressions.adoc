...
In either case, a `#subquery#` occurs as the argument of one of the following special quantifier functions.

[cols="10,1,~"]
|===
| Quantifier | Kind | Interpretation

| `exists` | Existence | Satisfied if the result list of the subquery is nonempty.
| `not exists` | Existence | Satisfied if the result list of the subquery is empty.
| `any` or `some` | Quantified comparison | Satisfied if the result list of the subquery contains at least one value which would make the equality or inequality expression evaluate to true if it occurred as the right operand in place of the subquery.
| `all` | Quantified comparison | Satisfied if every value belonging to the result list of the subquery would make the equality or inequality expression evaluate to true if it occurred as the right operand in place of the subquery.
|===
