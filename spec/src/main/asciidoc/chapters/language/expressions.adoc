...
If the first argument of `#extract()#` is `#DATE#`, the function application is assigned the type `LocalDate`. If the first argument of `#extract()#` is `#TIME#`, the function call is assigned the type `LocalTime`.
...
In the core language, the only allowed aggregate function application is the expression `count(this)`, as specified below in <<select-clause>>.
