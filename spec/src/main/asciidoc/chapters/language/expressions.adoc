...
- <<equality-and-inequality,equality and inequality>> operator expressions, and
...
- the `not` keyword is missing, and the value produced by evaluating the left operand does not occur in the result list of the subquery, or
- the `not` keyword occurs, and the value produced by evaluating the left operand does occur in the result list of the subquery.
...
- the `not` keyword is missing, and the value produced by evaluating the left operand is not a member of the value assigned to the parameter, or
- the `not` keyword occurs, and the value produced by evaluating the left operand is a member of the value assigned to the parameter.
...
A `like` expression, with syntax given by `like_expression` is satisfied when its left operand evaluates to a non-null value and:
