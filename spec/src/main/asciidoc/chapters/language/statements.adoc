...
Each statement is built from a sequence of _clauses_. The beginning of a clause is identified by a keyword: `from`, `where`, `select`, `set`, or `order`.
...
The `from` clause is optional in `select` statements. When it is missing, the queried entity is determined by some defaulting rule defined externally to this specification, for example:

1. a result type might be supplied via an API cainvocationll (for example, via `createQuery()` in Jakarta Persistence), or
